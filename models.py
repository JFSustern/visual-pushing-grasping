...
class place_net(nn.Module):
    ...
    def __init__(self, use_cuda):
        ...
        # Construct network branch for placement
        self.placenet = nn.Sequential(OrderedDict([
            ('place-norm0', nn.BatchNorm2d(2048)),
            ('place-relu0', nn.ReLU(inplace=True)),
            ('place-conv0', nn.Conv2d(2048, 64, kernel_size=1, stride=1, bias=False)),
    ...
    def forward(self, input_color_data, input_depth_data, is_volatile=False, specific_rotation=-1):
        ...
        if is_volatile:
            ...
            for rotate_idx in range(self.num_rotations):
                ...
                interm_feat.append([interm_place_feat])
                ...
                # Forward pass through branches, undo rotation on output predictions, upsample results
                output_prob.append([nn.Upsample(scale_factor=16, mode='bilinear').forward(F.grid_sample(self.placenet(interm_place_feat), flow_grid_after, mode='nearest'))])
            ...
            interm_place_depth_feat = self.place_depth_trunk.features(rotate_depth)
            interm_place_feat = torch.cat((interm_place_color_feat, interm_place_depth_feat), dim=1)
            self.interm_feat.append([interm_place_feat])
            ...
            # Forward pass through branches, undo rotation on output predictions, upsample results
            self.output_prob.append([nn.Upsample(scale_factor=16, mode='bilinear').forward(F.grid_sample(self.placenet(interm_place_feat), flow_grid_after, mode='nearest'))])
